...
def cut_roi(frame, roi, mode):
    if mode == "min_max":
        p1 = roi.position_min.astype(int)
        p1 = clip(p1, [0, 0], [frame.shape[-1], frame.shape[-2]])
        p2 = roi.position_max.astype(int)
...
def cut_rois(frame, rois, mode="def"):
    return [cut_roi(frame, roi, mode) for roi in rois]
