...
class VehicleLicenceDetector(Module):
    class Result:
        OUTPUT_SIZE = 7
    ...
    def __init__(self, model, confidence_threshold=0.5, roi_scale_factor=1.15):
        super(VehicleLicenceDetector, self).__init__(model)
    ...
    def enqueue(self, input):
        return super(VehicleLicenceDetector, self).enqueue({self.input_blob: input})
    ...
    def get_roi_proposals(self, frame):
        ...
        for output in outputs[0][0]:
            result = VehicleLicenceDetector.Result(output)
            ...
            if result.label == 1:
                ...
                license_plates.append(result)
        return vehicles, license_plates
