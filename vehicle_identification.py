...
def build_argparser():
    ...
    models.add_argument('-m_vl', metavar="PATH", default="models/vehicle-license-plate-detection-barrier-0106.xml",
                        help="Path to the Vehicle & Licence Plate Detection model XML file")
    ...
    infer.add_argument('-d_vl', default='CPU', choices=DEVICE_KINDS,
                       help="(optional) Target device for the " \
                       "Vehicle & Licence Plate Detection model (default: %(default)s)")
...
class FrameProcessor:
    ...
    def __init__(self, args):
        used_devices = set([args.d_vl, args.d_le, args.d_va])
        ...
        vehicle_licence_detector_net = self.load_model(args.m_vl)
        ...
        if args.vl_input_height and args.vl_input_width :
            vehicle_licence_detector_net.reshape({"data": [1, 3, args.vl_input_height,args.vl_input_width]})
            

        licence_extractor_net = self.load_model(args.m_le)
        vehicle_attr_detection_net = self.load_model(args.m_va)
        
        self.vehicle_licence_detector = VehicleLicenceDetector(vehicle_licence_detector_net,
        ...
        self.vehicle_licence_detector.deploy(args.d_vl, context)
    ...
    def process(self, frame):
        ...
        self.vehicle_licence_detector.clear()
        self.licence_extractor.clear()
        self.vehicle_attr_detection.clear()

        
        self.vehicle_licence_detector.start_async(frame)
        vehicle_rois, licence_rois = self.vehicle_licence_detector.get_roi_proposals(frame)
